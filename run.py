...
if __name__ == '__main__':
    ...
    parser.add_argument('--camera', action='store_true')  # Add camera option
    ...
    if args.camera:
        ...
        while True:
            ...
            depth_color = process_image(frame)

            os.makedirs(args.outdir, exist_ok=True)
            filename = 'camera_frame_img_depth.png'

            split_region = np.ones((frame.shape[0], margin_width, 3), dtype=np.uint8) * 255
            combined_results = cv2.hconcat([frame, split_region, depth_color])
        ...
        for filename in tqdm(filenames):
            ...
            depth_color = process_image(image)
            ...
            caption_space = np.ones((caption_height, combined_results.shape[1], 3), dtype=np.uint8) * 255
            captions = ['Raw image', 'Depth Anything']
            segment_width = raw_image.shape[1] + margin_width
            for i, caption in enumerate(captions):
                # Calculate text size
                text_size = cv2.getTextSize(caption, font, font_scale, font_thickness)[0]

                # Calculate x-coordinate to center the text
                text_x = int((segment_width * i) + (raw_image.shape[1] - text_size[0]) / 2)
